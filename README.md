...
Use `macos-intel` for Intel Macs and `macos-apple-silicon` for M1 / M2 / M3 / M4 Macs. macOS packages contain `deadline-survivors.app`; Windows and Linux packages contain a standalone executable. Push a tag like `v0.2.5` to create a GitHub Release with these assets.

CI runs a packaged binary smoke test on every release build:
